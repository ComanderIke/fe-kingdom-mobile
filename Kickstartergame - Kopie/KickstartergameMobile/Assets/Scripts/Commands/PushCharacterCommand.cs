...
namespace Assets.Scripts.Commands
{
    class PushCharacterCommand : Command
    {
        ...
        public override void Execute()
        {
            ...
            if (MainScript.GetInstance().gridManager.GridLogic.IsTileAccessible(pos))
            {
                if (MainScript.GetInstance().gridManager.Tiles[(int)pos.x, (int)pos.y].character != null)
                {
                    MainScript.GetInstance().gridManager.Tiles[(int)pos.x, (int)pos.y].character.MoveActions.Push(direction);
                }
                if (MainScript.GetInstance().gridManager.GridLogic.IsTileAccessible(pos, unit))

...
namespace Assets.Scripts.AI.AttackReactions
{
    public class RevengeStrike : AttackReaction
    {
        ...
        IEnumerator Revenge()
        {
            ...
            Attack(User, MainScript.GetInstance().GetSystem<GridSystem>().GetTileFromVector2(TargetPositions[0]).character);
        ...
        private void Attack(LivingObject attacker, LivingObject defender)
        {
            if (DoesAttackHit(attacker, defender))
            {
                ...
                MainScript.GetInstance().GetSystem<UISystem>().attackUIController.ShowCounterDamageText(damage);
            ...
            {
                MainScript.GetInstance().GetSystem<UISystem>().attackUIController.ShowCounterMissText();
            }
        }

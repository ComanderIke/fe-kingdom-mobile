...
namespace Game.GameActors.Units.Skills
{
    ...
    [CreateAssetMenu(menuName = "GameData/Skills/Active/PositionTarget", fileName = "PositionTargetSkillMixin")]
        ...
        public virtual void Activate(Unit user, Tile[,] tiles, int x, int y)
        {
           ...
           var chosenPrevPos = previousPos[0];
            ...
            foreach (var pos in targetPositions)
            {
                    ...
                    yPosition < tiles.GetLength(1))
                {
                    ...
                    foreach (SkillEffectMixin effect in SkillEffects)
                    {
                        ...
                        else if (effect is UnitTargetSkillEffectMixin unitTargetSkillEffectMixin&&(Unit)tiles[xPosition,yPosition].GridObject!=null)
                        {
                        
                            unitTargetSkillEffectMixin.Activate((Unit)tiles[xPosition,yPosition].GridObject, user,skill.Level);
                        }
                    }
            ...
            if (jump)
            {
                ...
                {
                    targetPos = new Vector2Int((int)(unitPos.x+targetPositions[index].x),(int)(unitPos.y+ targetPositions[index].y));
                    Debug.Log("Jump to: "+targetPos);
                }
                Debug.Log("Direction: "+direction);
                targetPos = FindValidTargetPosition(user, tiles, targetPos, gridSystem, index, unitPos, targetPositions);
        ...
        {
            ServiceProvider.Instance.GetSystem<GridSystem>().SetUnitPosition(user, targetPos.x, targetPos.y, true ,false);
            Vector2 tiledifference = targetPos - unitPos;
            Debug.Log("===PosDifference: "+tiledifference+" "+tiledifference.magnitude);
            LeanTween.move(user.GameTransformManager.GameObject, new Vector2(targetPos.x, targetPos.y), tiledifference.magnitude*speedPerTile)
                .setEaseOutQuad();
            if (!String.IsNullOrEmpty(Animation))
            {
                user.GameTransformManager.UnitAnimator.SetAlternateWalk(true);
                MonoUtility.DelayFunction(()=> user.GameTransformManager.UnitAnimator.SetAlternateWalk(false),tiledifference.magnitude*speedPerTile);
            }
        }
        ...
        private  Vector2Int FindValidTargetPosition(Unit user, Tile[,] tiles, Vector2Int targetPos, GridSystem gridSystem,
        ...
        protected List<Vector2Int> GetTargetPositions(int level, Vector2Int direction = default)
        {
            ...
            if (GetSize(level) > 0)
            {
                ...
                else
                {
                    ...
                    {
                        if (TargetArea == SkillTargetArea.Line && direction.x!=0|| TargetArea == SkillTargetArea.Cross ||

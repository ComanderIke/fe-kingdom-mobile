    ...
    public class MovementSkillEffectMixin : UnitTargetSkillEffectMixin
    {
        ...
        public override void Activate(Unit target, Unit caster, int level)
        {
             ...
             if (rescueTarget)
             {
                 ...
                 Rescue(target, caster);
                 return;
             }
             if (swapPositions)
             {
                 SwapPositions(target, caster);
                 return;
             }
        ...
        private void MoveRandomly(Unit target)
        {
            var gridSystem = ServiceProvider.Instance.GetSystem<GridSystem>();
            ...
            int cnt = 0;
            Debug.Log("Rand: " + rand);
            while ((rand.x == 0 && rand.y == 0) || gridSystem.IsOutOfBounds(newPosition.x, newPosition.y) ||
                   !gridSystem.GridLogic.IsTileFreeAndNotBlocked(newPosition.x, newPosition.y))
            {
        ...
        private static void SwapPositions(Unit target, Unit caster)
        {
            var gridSystem = ServiceProvider.Instance.GetSystem<GridSystem>();
        ...
        private void Rescue(Unit target, Unit caster)
        {

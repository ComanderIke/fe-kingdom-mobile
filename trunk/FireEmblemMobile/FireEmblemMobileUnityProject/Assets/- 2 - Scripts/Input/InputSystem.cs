...
public class InputSystem : MonoBehaviour, EngineSystem {
    ...
    void EndDrag()
    {
        ...
        {
            RaycastHit latestHit = raycastManager.GetLatestHit();
            if (latestHit.collider.gameObject.tag == "Grid")
            {
                onEndDragOverGrid((int)gridPos.x, (int)gridPos.y);
            }
            else if (latestHit.collider.gameObject.GetComponent<UnitController>() != null)
            {
                Unit draggedOverUnit = latestHit.collider.gameObject.GetComponent<UnitController>().Unit;

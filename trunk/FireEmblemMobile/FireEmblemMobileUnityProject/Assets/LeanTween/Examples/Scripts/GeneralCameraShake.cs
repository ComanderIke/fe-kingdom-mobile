...
namespace LeanTween.Examples.Scripts
{
	public class GeneralCameraShake : MonoBehaviour {
		...
		void bigGuyJump(){
			...
			Framework.LeanTween.moveY(avatarBig, height, 1f).setEase(Framework.LeanTweenType.easeInOutQuad).setOnComplete( ()=>{
				Framework.LeanTween.moveY(avatarBig, 0f, 0.27f).setEase(Framework.LeanTweenType.easeInQuad).setOnComplete( ()=>{
				...
				**************/
					...
					Framework.LTDescr shakeTween = Framework.LeanTween.rotateAroundLocal( gameObject, Vector3.right, shakeAmt, shakePeriodTime)
						.setEase( Framework.LeanTweenType.easeShake ) // this is a special ease that is good for shaking
						.setLoopClamp()
						.setRepeat(-1);
					...
					).setEase(Framework.LeanTweenType.easeOutQuad);

...
namespace LeanTween.Examples.Scripts
{
	public class PathSplineTrack : MonoBehaviour {
		...
		private Framework.LTSpline track;
		...
		void Start () {
			...
			track = new Framework.LTSpline( new Vector3[] {trackOnePoints[0].position, trackOnePoints[1].position, trackOnePoints[2].position, trackOnePoints[3].position, trackOnePoints[4].position, trackOnePoints[5].position, trackOnePoints[6].position} );
		...
		void Update () {
			...
			if(Input.anyKeyDown){
				...
				Framework.LeanTween.moveLocalX(carInternal, (trackIter-1)*6f, 0.3f).setEase(Framework.LeanTweenType.easeOutBack);
		...
		void OnDrawGizmos(){
			Framework.LTSpline.drawGizmo( trackOnePoints, Color.red);
		}

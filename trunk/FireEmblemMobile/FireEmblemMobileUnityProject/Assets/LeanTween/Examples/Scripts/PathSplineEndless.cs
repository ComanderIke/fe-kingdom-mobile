...
namespace LeanTween.Examples.Scripts
{
	public class PathSplineEndless : MonoBehaviour {
		...
		void addRandomTrackPoint(){
			...
			Framework.LeanTween.rotateAround( tree, Vector3.forward, 0f, 1f).setFrom( zIter%2==0 ? 180f : -180f).setEase(Framework.LeanTweenType.easeOutBack);
		...
		void refreshSpline(){
			track = new Framework.LTSpline( trackPts.ToArray() );

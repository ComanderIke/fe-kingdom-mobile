...
namespace LeanTween.Examples.Scripts
{
	public class GeneralAdvancedTechniques : MonoBehaviour {
		...
		void Start () {
			...
			}).setEase(Framework.LeanTweenType.easeInOutExpo).setLoopPingPong();
			...
			for(int i = 0; i < avatarSpeed.Length; i++)
				Framework.LeanTween.moveLocalZ( avatarSpeed[i], (i+1)*5f, 1f).setSpeed(6f).setEase(Framework.LeanTweenType.easeInOutExpo).setLoopPingPong(); // any time you set the speed it overrides the time value

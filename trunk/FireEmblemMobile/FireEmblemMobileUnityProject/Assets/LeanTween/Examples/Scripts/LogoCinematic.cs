...
namespace LeanTween.Examples.Scripts
{
	public class LogoCinematic : MonoBehaviour {
		...
		void Start () {
			...
			Framework.LeanTween.moveLocalX(tween, tween.transform.localPosition.x+15f, 0.4f).setEase(Framework.LeanTweenType.linear).setDelay(0f).setOnComplete( playBoom );

			// Drop Down tween down
			tween.transform.RotateAround(tween.transform.position, Vector3.forward, -30f);
			Framework.LeanTween.rotateAround(tween, Vector3.forward, 30f, 0.4f).setEase(Framework.LeanTweenType.easeInQuad).setDelay(0.4f).setOnComplete( playBoom );

			// Drop Lean In
			lean.transform.position += Vector3.up * 5.1f;
			Framework.LeanTween.moveY(lean, lean.transform.position.y-5.1f, 0.6f).setEase(Framework.LeanTweenType.easeInQuad).setDelay(0.6f).setOnComplete( playBoom );
		}

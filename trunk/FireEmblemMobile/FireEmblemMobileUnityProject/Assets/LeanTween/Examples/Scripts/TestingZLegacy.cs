...
//		alphaExample();
			...
			if(exampleIter==0){
				...
				if(timingType==TimingType.VariableTimeScale){
					descrTimeScaleChangeId = Framework.LeanTween.value( gameObject, 0.01f, 10.0f, 3f).setOnUpdate( (float val)=>{
						...
						Time.timeScale = val;
					}).setEase(Framework.LeanTweenType.easeInQuad).setUseEstimatedTime(true).setRepeat(-1).id;
				}else{
		...
		public void updateValue3Example(){
			...
			Framework.LeanTween.value( gameObject, updateValue3ExampleCallback, new Vector3(0.0f, 270.0f, 0.0f), new Vector3(30.0f, 270.0f, 180f), 0.5f ).setEase(Framework.LeanTweenType.easeInBounce).setRepeat(2).setLoopPingPong().setOnUpdateVector3(updateValue3ExampleUpdate).setUseEstimatedTime(useEstimatedTime);
		}
		...
		public void loopTestClamp(){
			...
			Framework.LeanTween.scaleZ( cube1, 4.0f, 1.0f).setEase(Framework.LeanTweenType.easeOutElastic).setRepeat(7).setLoopClamp().setUseEstimatedTime(useEstimatedTime);//
		}
		...
		public void loopTestPingPong(){
			...
			Framework.LeanTween.scaleY( cube2, 4.0f, 1.0f ).setEase(Framework.LeanTweenType.easeOutQuad).setLoopPingPong(4).setUseEstimatedTime(useEstimatedTime);
		...
		public void colorExample(){
			...
			Framework.LeanTween.color( lChar, new Color(1.0f,0.0f,0.0f,0.5f), 0.5f ).setEase(Framework.LeanTweenType.easeOutBounce).setRepeat(2).setLoopPingPong().setUseEstimatedTime(useEstimatedTime);
		}
		...
		public void moveOnACurveExample(){
			...
			Framework.LeanTween.move( ltLogo, path, 1.0f ).setEase(Framework.LeanTweenType.easeOutQuad).setOrientToPath(true).setUseEstimatedTime(useEstimatedTime);
		}
		...
		public void rotateExample(){
			...
			Framework.LeanTween.rotate( ltLogo, new Vector3(0f,360f,0f), 1f).setEase(Framework.LeanTweenType.easeOutQuad).setOnComplete(rotateFinished).setOnCompleteParam(returnParam).setOnUpdate(rotateOnUpdate).setUseEstimatedTime(useEstimatedTime);
		}
		...
		public void scaleExample(){
			...
			Framework.LeanTween.scale( ltLogo, new Vector3(currentScale.x+0.2f,currentScale.y+0.2f,currentScale.z+0.2f), 1f ).setEase(Framework.LeanTweenType.easeOutBounce).setUseEstimatedTime(useEstimatedTime);
		}
		...
		public void updateValueExample(){
			...
			pass.Add("message", "hi");
			Framework.LeanTween.value( gameObject, (Action<float, object>)updateValueExampleCallback, ltLogo.transform.eulerAngles.y, 270f, 1f ).setEase(Framework.LeanTweenType.easeOutElastic).setOnUpdateParam(pass).setUseEstimatedTime(useEstimatedTime);
		}
		...
		public void delayedCallExampleCallback(){
			...
			Framework.LeanTween.scale( ltLogo, new Vector3(currentScale.x-0.2f,currentScale.y-0.2f,currentScale.z-0.2f), 0.5f ).setEase(Framework.LeanTweenType.easeInOutCirc).setUseEstimatedTime(useEstimatedTime);
		}
		...
		public void punchTest(){
			Framework.LeanTween.moveX( ltLogo, 7.0f, 1.0f ).setEase(Framework.LeanTweenType.punch).setUseEstimatedTime(useEstimatedTime);
		}
	}
}
